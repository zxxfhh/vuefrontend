...
export const useMultiTagsStore = defineStore("pure-multiTags", {
  state: () => ({
    // 存储标签页信息（路由信息）
    multiTags: storage.getItem<StorageConfigs>(
      `${responsiveStorageNameSpace()}configure`
    )?.multiTagsCache
      ? storage.getItem<StorageConfigs>(`${responsiveStorageNameSpace()}tags`)
      : ([
          ...routerArrays,
    ...
    multiTagsCache: storage.getItem<StorageConfigs>(
      `${responsiveStorageNameSpace()}configure`
    )?.multiTagsCache
  }),
  getters: {
  ...
  actions: {
    multiTagsCacheChange(multiTagsCache: boolean) {
      ...
      if (multiTagsCache) {
        storage.setItem(`${responsiveStorageNameSpace()}tags`, this.multiTags);
      } else {
        storage.removeItem(`${responsiveStorageNameSpace()}tags`);
      }
    },
    tagsCache(multiTags) {
      this.getMultiTagsCache &&
        storage.setItem(`${responsiveStorageNameSpace()}tags`, multiTags);
    },
    handleTags<T>(
      mode: string,
      value?: T | multiType,
